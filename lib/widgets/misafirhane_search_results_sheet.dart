...
class _MisafirhaneSearchResultsPanelState
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // İlk frame sonrası natives + scroll (setState artık güvenli).
      ...
      SliverList(
        delegate: SliverChildBuilderDelegate(
          (ctx, index) {
            ...
            final wrapped = isHl
                ? _SearchHighlightFlash(totalMs: 5000, child: row)
                : row;
...
class _SearchHighlightFlash extends StatefulWidget {
  const _SearchHighlightFlash({required this.child, this.totalMs = 10000});
