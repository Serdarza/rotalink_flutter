...
abstract final class GithubFiyatDataSource {
  ...
  static Uri get _uri => GithubFiyatConfig.uri;

  /// HEAD — gövde indirilmez.
  static Future<String?> fetchRemoteVersion() async {
    try {
      ...
      if (res.statusCode != 200) {
        _log('Fiyat sürüm kontrolü başarısız: HTTP ${res.statusCode}');
        return null;
      }
      ...
      _log('Fiyat sürüm kontrolü hatası: $e', st);
      return null;
    }
  }

  static Future<String?> fetchFiyatlarFromGitHub() async {
    try {
      final res = await http
      ...
      if (res.statusCode != 200) {
        _log(
          'Fiyatlar indirilemedi: HTTP ${res.statusCode} '
          '(${GithubFiyatConfig.rawUrl})',
        );
        return null;
      }
      ...
      _log('Fiyatlar indirme hatası: $e', st);
      return null;
    }
  }

  static void _log(String message, [StackTrace? st]) {
    debugPrint('[GithubFiyatDataSource] $message');
