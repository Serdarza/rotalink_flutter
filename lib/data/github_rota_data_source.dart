...
/// GitHub Raw üzerinden master_database_updated.json indirme ve sürüm kontrolü.
abstract final class GithubRotaDataSource {
  ...
  static Uri get _uri => GithubRotaConfig.databaseUri;
  ...
  static Future<String?> fetchRemoteVersion() async {
    try {
      ...
      if (res.statusCode != 200) {
        _log('GitHub sürüm kontrolü başarısız: HTTP ${res.statusCode}');
        return null;
      }
      ...
      _log('GitHub sürüm kontrolü hatası: $e', st);
      return null;
    }
  }

  /// GitHub Raw linkinden tüm veritabanı JSON'unu indirir.
  static Future<String?> fetchMasterDatabaseFromGitHub() async {
    try {
      ...
      if (res.statusCode != 200) {
        _log(
          'GitHub veritabanı indirilemedi: HTTP ${res.statusCode} '
          '(${GithubRotaConfig.rawDatabaseUrl})',
        );
        return null;
      }
      ...
      if (body.isEmpty) {
        _log('GitHub yanıtı boş.');
        return null;
      }
      return body;
    } catch (e, st) {
      _log('GitHub veritabanı indirme hatası (internet veya bağlantı): $e', st);
      return null;
    }
  }
